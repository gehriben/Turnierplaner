...
export class RestService{
  ...
  put(object : any, url : string): Observable<any> {
    return this.http.put<any>(this.endpoint + url, JSON.stringify(object), this.httpOptions).pipe();
  }

  delete(id : number, url : string): Observable<any> {
     return this.http.delete<any>(this.endpoint + url + id, this.httpOptions).pipe();
  }
}

package ch.zhaw.psit4.turnier.persistence.service;

import ch.zhaw.psit4.turnier.model.Ranking;
import ch.zhaw.psit4.turnier.model.User;
import ch.zhaw.psit4.turnier.persistence.repository.RankingRepository;
import ch.zhaw.psit4.turnier.persistence.repository.UserRepository;
import org.springframework.boot.test.mock.mockito.MockBean;
import org.springframework.data.repository.CrudRepository;

import java.util.ArrayList;
import java.util.Collections;

public class RankingServiceUnitTest extends BaseServiceUnitTest<Ranking, Integer> {

    @MockBean
    private RankingRepository groupRepository;

    @Override
    public CrudRepository<Ranking, Integer> getCrudRepository() {
        return groupRepository;
    }

    @Override
    public ArrayList<Ranking> getObjList() {
        return new ArrayList<>(Collections.singleton(new Ranking()));
    }
}
